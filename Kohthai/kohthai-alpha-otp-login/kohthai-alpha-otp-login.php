<?php
/**
 * Plugin Name:       Kohthai Alpha SMS OTP Login
 ...
 * Version:           1.0.0
 * Author:            Kohthai / Smart Living Bangladesh
 * Author URI:        https://kohthaibd.com/
 * License:           GPL-2.0+
 * License URI:       http://www.gnu.org/licenses/gpl-2.0.txt
 * Text Domain:       kohthai-alpha-otp-login
...
define( 'KT_ALPHA_OTP_VERSION', '1.0.0' );
define( 'KT_ALPHA_OTP_FILE', __FILE__ );
define( 'KT_ALPHA_OTP_PATH', plugin_dir_path( __FILE__ ) );
define( 'KT_ALPHA_OTP_URL', plugin_dir_url( __FILE__ ) );

/** Our own settings option. */
define( 'KT_ALPHA_OTP_OPTION', 'kt_alpha_otp' );

/** The Alpha SMS plugin's option (we read its API key + sender id from here). */
define( 'KT_ALPHA_OTP_ALPHA_OPTION', 'alpha_sms' );

require_once KT_ALPHA_OTP_PATH . 'includes/class-kt-alpha-otp-sms.php';
require_once KT_ALPHA_OTP_PATH . 'includes/class-kt-alpha-otp-session.php';
require_once KT_ALPHA_OTP_PATH . 'includes/class-kt-alpha-otp-login.php';

if ( is_admin() ) {
	require_once KT_ALPHA_OTP_PATH . 'admin/class-kt-alpha-otp-admin.php';
}
 ...
 * @return array
 */
function kt_alpha_otp_default_options() {
	return array(
		'enabled'          => 'yes',
 ...
 * @return array
 */
function kt_alpha_otp_get_options() {
	$stored = get_option( KT_ALPHA_OTP_OPTION, array() );
	if ( ! is_array( $stored ) ) {
		$stored = array();
	}
	return array_merge( kt_alpha_otp_default_options(), $stored );
}

/**
 * Activation: seed default options if none exist yet.
 */
function kt_alpha_otp_activate() {
	if ( false === get_option( KT_ALPHA_OTP_OPTION, false ) ) {
		add_option( KT_ALPHA_OTP_OPTION, kt_alpha_otp_default_options() );
	}
}
register_activation_hook( __FILE__, 'kt_alpha_otp_activate' );

/**
 * Boot the plugin after all plugins are loaded so WooCommerce + Alpha SMS are available.
 */
function kt_alpha_otp_boot() {
	// WooCommerce is required for the account login form hooks.
	if ( ! class_exists( 'WooCommerce' ) ) {
		return;
	}

	$front = new KT_Alpha_OTP_Login();
	$front->init();

	if ( is_admin() ) {
		$admin = new KT_Alpha_OTP_Admin();
		$admin->init();
	}
}
add_action( 'plugins_loaded', 'kt_alpha_otp_boot', 20 );
