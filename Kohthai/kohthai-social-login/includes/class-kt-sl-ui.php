...
class KT_SL_UI {
	...
	public static function init() {
		...
		// Outside and above the login form. Useful if another plugin ever hides or
		// replaces the email/password form, since anything rendered *inside* that
		// form disappears with it.
		...
		add_shortcode( 'kohthai_social_login', array( __CLASS__, 'shortcode' ) );
	}

	public static function enqueue() {
		if ( ! KT_SL_Options::active_providers() ) {
			return;
		}
		wp_register_style( 'kohthai-social-login', KT_SL_URL . 'assets/kohthai-social-login.css', array(), KT_SL_VERSION );
		wp_enqueue_style( 'kohthai-social-login' );
		...
		$css = ':root{--kt-sl-size:' . $size . 'px;--kt-sl-radius:' . $radius
			 . ';--kt-sl-radius-w:' . $radius_wide . ';}';
		wp_add_inline_style( 'kohthai-social-login', $css );
	}
	...
	public static function render() {
		...
		$providers = KT_SL_Options::active_providers();
		if ( ! $providers ) {
			return;
		}
		...
		echo '<div class="kt-sl kt-sl-align-' . esc_attr( $o['align'] ) . ( $label ? ' kt-sl-labelled' : '' ) . '">';

		if ( trim( (string) $o['title'] ) !== '' ) {
			echo '<div class="kt-sl-title"><span>' . esc_html( $o['title'] ) . '</span></div>';
		}

		echo '<div class="kt-sl-buttons">';
		foreach ( $providers as $p ) {
			$name = $brands[ $p ][0];
			$text = $brands[ $p ][1];
			printf(
				'<a class="kt-sl-btn kt-sl-%1$s" href="%2$s" rel="nofollow noopener" aria-label="%3$s" title="%3$s">%4$s%5$s</a>',
				esc_attr( $p ),
				esc_url( KT_SL_OAuth::start_url( $p, $here ) ),
				esc_attr( $text ),
				self::icon( $p ),
				$label ? '<span class="kt-sl-btn-text">' . esc_html( $text ) . '</span>' : ''
			);
		}
		echo '</div></div>';
	}

	/** Shortcode form, e.g. [kohthai_social_login] on a custom page. */
	...
	private static function icon( $provider ) {
		if ( 'google' === $provider ) {
			return '<svg class="kt-sl-ico" viewBox="0 0 48 48" aria-hidden="true" focusable="false">'
		...
		return '<svg class="kt-sl-ico" viewBox="0 0 24 24" aria-hidden="true" focusable="false">'
	...
	private static function current_url() {
		...
		return esc_url_raw( remove_query_arg( 'kt_sl_error', home_url( $uri ) ) );
	}

	/** Render the ?kt_sl_error=... message as a WooCommerce-style notice. */
	public static function maybe_error() {
		if ( empty( $_GET['kt_sl_error'] ) ) {
			return;
		}
		...
		// choosing inside our error box — see KT_SL_OAuth::messages().
		$code = sanitize_key( wp_unslash( $_GET['kt_sl_error'] ) );
		$msg  = KT_SL_OAuth::message( $code );
		if ( $msg === '' ) {
			return;
		}
		echo '<div class="woocommerce-error kt-sl-error" role="alert">' . esc_html( $msg ) . '</div>';
	}
}
