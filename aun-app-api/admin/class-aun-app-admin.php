...
class AUN_App_Admin {
	...
	public function menu() {
		...
		add_submenu_page( 'aun-app', 'AUN App Device Lookup', 'Device lookup', self::CAP, 'aun-app-lookup', array( $this, 'page_lookup' ) );
	 ...
	 */
	/**
	 * AUN App → Device lookup.
	 *
	 * Asks the ERP the two questions the app asks — by serial and by phone —
	 * and shows exactly what came back.
	 *
	 * ⚠️ This exists because "the app cannot find my device" has at least five
	 * different causes that look identical from a phone, and there is no shell
	 * on this server to check any of them. Every one is answerable from these
	 * two responses:
	 *
	 *  - the phone lookup lists what the ERP will EVER give the app for that
	 *    customer. A serial missing from that list is missing from the app;
	 *  - a serial found by phone but NOT by serial means the note is there but
	 *    the token does not match exactly (a prefix like "SN:" on the same line,
	 *    or a stray character);
	 *  - nothing at all means the sell line has no serial in its note, or the
	 *    sale is not `final`.
	 *
	 * ⚠️ It deletes the lookup transient first. `lookup_serial()` caches MISSES
	 * for ten minutes, so without this a serial fixed in the ERP still reads
	 * "not found" for ten minutes and the fix looks like it failed.
	 */
	public function page_lookup() {
		...
		echo '<div class="wrap"><h1>Device lookup</h1>';
		echo '<p style="max-width:820px">Runs the same two lookups the app runs, against the live ERP, '
			. 'and shows the raw answer. Use it when a customer says the app cannot find their projector.</p>';

		if ( ! class_exists( 'AUN_App_ERP' ) || ! AUN_App_ERP::configured() ) {
			echo '<div class="notice notice-error"><p><strong>The ERP connection is not configured.</strong> '
				. 'Set the base URL and secret in AUN App → Settings first.</p></div></div>';
			return;
		}

		$mode  = isset( $_POST['mode'] ) ? sanitize_text_field( wp_unslash( $_POST['mode'] ) ) : 'serial';
		$value = isset( $_POST['value'] ) ? sanitize_text_field( wp_unslash( $_POST['value'] ) ) : '';
		$run   = isset( $_POST['aun_lookup_nonce'] )
			&& wp_verify_nonce( $_POST['aun_lookup_nonce'], 'aun_app_lookup' )
			&& '' !== trim( $value );

		echo '<form method="post" style="margin:16px 0;max-width:820px">';
		wp_nonce_field( 'aun_app_lookup', 'aun_lookup_nonce' );
		echo '<select name="mode" style="height:32px">';
		echo '<option value="serial"' . selected( $mode, 'serial', false ) . '>By serial / box barcode</option>';
		echo '<option value="phone"' . selected( $mode, 'phone', false ) . '>By phone number</option>';
		echo '</select> ';
		echo '<input type="text" name="value" value="' . esc_attr( $value ) . '" class="regular-text" '
			. 'placeholder="e.g. 701254802893 or 01712345678" /> ';
		submit_button( 'Look up', 'primary', 'submit', false );
		echo '</form>';
		...
		if ( 'phone' === $mode ) {
			$canonical = class_exists( 'AUN_App_Phone' ) ? AUN_App_Phone::normalize( $value ) : '';
			if ( ! $canonical ) {
				echo '<div class="notice notice-error"><p>That is not a valid Bangladeshi mobile number.</p></div></div>';
				return;
			}
			echo '<p><strong>Asking the ERP for:</strong> <code>' . esc_html( $canonical ) . '</code> '
				. '(matched on the last 10 digits of the CONTACT record&rsquo;s mobile).</p>';

			$rows = AUN_App_ERP::lookup_phone( $canonical );
			if ( is_wp_error( $rows ) ) {
				echo '<div class="notice notice-error"><p><strong>ERP error:</strong> '
					. esc_html( $rows->get_error_message() ) . '</p></div></div>';
				return;
			}
			self::render_lookup_rows( (array) $rows );
			echo '</div>';
			return;
		}

		$serial = strtoupper( $value );
		echo '<p><strong>Asking the ERP for serial:</strong> <code>' . esc_html( $serial ) . '</code></p>';

		if ( ! preg_match( '/^[A-Z0-9\-_\/]{3,60}$/', $serial ) ) {
			echo '<div class="notice notice-error"><p><strong>This serial cannot be looked up at all.</strong> '
				. 'The API accepts only letters, digits, <code>-</code>, <code>_</code> and <code>/</code>, '
				. '3&ndash;60 characters. A space or a colon in the value stops it before it reaches the ERP.</p></div>';
		}

		delete_transient( 'aun_app_erp_s_' . md5( $serial ) );

		$sale = AUN_App_ERP::lookup_serial( $serial );
		if ( is_wp_error( $sale ) ) {
			echo '<div class="notice notice-error"><p><strong>ERP error:</strong> '
				. esc_html( $sale->get_error_message() ) . '</p></div></div>';
			return;
		}

		if ( ! $sale ) {
			echo '<div class="notice notice-warning"><p><strong>The ERP has no finalised sale line whose '
				. 'note contains exactly this token.</strong></p></div>';
			echo '<p style="max-width:820px">In order of how often each one is the cause:</p>';
			echo '<ol style="max-width:820px">';
			echo '<li><strong>The serial is not in the sell line note.</strong> The app can only see serials '
				. 'typed into <em>sell_line_note</em> on the invoice line. A sale recorded without one is '
				. 'invisible to the app no matter how recent it is.</li>';
			echo '<li><strong>The note has the serial but not on its own.</strong> Notes are split on new '
				. 'lines, commas and semicolons &mdash; <em>not spaces</em>. So <code>SN: 701254802893</code> '
				. 'is ONE token and can never match <code>701254802893</code>. One bare serial per line.</li>';
			echo '<li><strong>The note holds a different number.</strong> The box sticker carries a 12-digit '
				. 'barcode and a shorter <code>SN:</code>. If the ERP has one and the customer types the '
				. 'other, they will not meet.</li>';
			echo '<li><strong>The sale is not <code>final</code></strong> (draft or quotation).</li>';
			echo '<li>The line was <strong>fully returned</strong>.</li>';
			echo '</ol>';
			echo '<p style="max-width:820px"><strong>Next step:</strong> look the customer up '
				. '<em>by phone</em> above. That lists every serial the ERP will give the app for them &mdash; '
				. 'if the unit is in that list it is cause 2 or 3; if it is not, it is cause 1, 4 or 5.</p>';
			echo '</div>';
			return;
		}

		self::render_lookup_rows( array( $sale ) );
		echo '</div>';
	}

	/** One table of ERP sale rows, with what the app would do about each. */
	private static function render_lookup_rows( array $rows ) {
		if ( empty( $rows ) ) {
			echo '<div class="notice notice-warning"><p><strong>The ERP returned nothing for this number.</strong></p></div>';
			echo '<p style="max-width:820px">Either no finalised sale is attached to a contact whose mobile '
				. 'ends in those 10 digits, or those sales carry no serials in their line notes. '
				. '&#9888; The match is on the <strong>contact record&rsquo;s</strong> mobile &mdash; a number '
				. 'typed only on the invoice, or a second number the customer used, will not be found.</p>';
			echo '<p style="max-width:820px">&#9888; The phone lookup also returns only the <strong>60 most '
				. 'recent sale lines</strong> for that contact, and at most 30 serials. For a dealer with a '
				. 'long history an older unit can fall off the end &mdash; look that one up by serial instead.</p>';
			return;
		}

		echo '<p><strong>' . count( $rows ) . '</strong> row(s) returned.</p>';
		echo '<table class="widefat striped" style="max-width:1000px"><thead><tr>'
			. '<th>Serial</th><th>Product</th><th>Sold</th><th>Invoice</th>'
			. '<th>Warranty</th><th>What the app does</th></tr></thead><tbody>';

		foreach ( $rows as $r ) {
			$note = array();
			if ( ! empty( $r['returned'] ) ) {
				$note[] = '<span style="color:#b32d2e"><strong>Returned</strong> &mdash; hidden from the app, '
					. 'and unlinked if it was already added.</span>';
			} elseif ( AUN_App_ERP::is_dealer_sale( $r ) ) {
				$note[] = 'Dealer sale &mdash; <strong>not</strong> auto-linked; the customer must register '
					. 'it manually (invoice photo required).';
			} else {
				$note[] = '<span style="color:#1a7f37">Direct sale &mdash; auto-links to the account on login '
					. 'and on find-by-phone.</span>';
			}
			if ( empty( $r['maintenance'] ) ) {
				$note[] = 'No dust-filter reminders (product_custom_field1 is not <code>MAINT_SMS</code>).';
			}

			$warranty = ( null === $r['warranty_duration'] || '' === $r['warranty_unit'] )
				? '<em>none set on the product</em>'
				: esc_html( $r['warranty_duration'] . ' ' . $r['warranty_unit'] );

			echo '<tr>';
			echo '<td><code>' . esc_html( $r['serial'] ) . '</code></td>';
			echo '<td>' . esc_html( $r['product_name'] ) . '</td>';
			echo '<td>' . esc_html( $r['sale_date'] ) . '</td>';
			echo '<td>' . esc_html( $r['invoice_no'] ) . '</td>';
			echo '<td>' . $warranty . '</td>'; // phpcs:ignore WordPress.Security.EscapeOutput
			echo '<td>' . implode( '<br>', $note ) . '</td>'; // phpcs:ignore WordPress.Security.EscapeOutput
			echo '</tr>';
		}
		echo '</tbody></table>';
		echo '<p style="max-width:820px">&#9888; These serials are exactly what the app will accept. If a '
			. 'customer is typing something that is not in this list, that is the problem &mdash; and note '
			. 'that the app&rsquo;s &ldquo;Type the number&rdquo; field accepts <strong>digits only</strong>, '
			. 'so a serial containing a letter or a dash cannot be typed at all.</p>';
	}
