...
class BREO_SL_UI {
	...
	public static function init() {
		...
		// NOTE on hook choice (Breo): the login buttons go at the END of the form, after
		// the phone-OTP block (Breo Alpha OTP Login renders at priority 20), so "Or
		// continue with" sits under the main sign-in action and stays visible on both
		// the OTP and the password step. 'woocommerce_register_form' still fires inside
		// the register form, before its submit button.
		if ( ! empty( $o['at_wc_login'] ) ) {
			add_action( 'woocommerce_login_form_end', array( __CLASS__, 'render' ), 30 );
		}
		...
		add_shortcode( 'breo_social_login', array( __CLASS__, 'shortcode' ) );
	}

	public static function enqueue() {
		if ( ! BREO_SL_Options::active_providers() ) {
			return;
		}
		wp_register_style( 'breo-social-login', BREO_SL_URL . 'assets/breo-social-login.css', array(), BREO_SL_VERSION );
		wp_enqueue_style( 'breo-social-login' );
		...
		$css = ':root{--breo-sl-size:' . $size . 'px;--breo-sl-radius:' . $radius
			 . ';--breo-sl-radius-w:' . $radius_wide . ';}';
		wp_add_inline_style( 'breo-social-login', $css );
	}
	...
	public static function render() {
		...
		$providers = BREO_SL_Options::active_providers();
		if ( ! $providers ) {
			return;
		}
		...
		echo '<div class="breo-sl breo-sl-align-' . esc_attr( $o['align'] ) . ( $label ? ' breo-sl-labelled' : '' ) . '">';

		if ( trim( (string) $o['title'] ) !== '' ) {
			echo '<div class="breo-sl-title"><span>' . esc_html( $o['title'] ) . '</span></div>';
		}

		echo '<div class="breo-sl-buttons">';
		foreach ( $providers as $p ) {
			$name = $brands[ $p ][0];
			$text = $brands[ $p ][1];
			printf(
				'<a class="breo-sl-btn breo-sl-%1$s" href="%2$s" rel="nofollow noopener" aria-label="%3$s" title="%3$s">%4$s%5$s</a>',
				esc_attr( $p ),
				esc_url( BREO_SL_OAuth::start_url( $p, $here ) ),
				esc_attr( $text ),
				self::icon( $p ),
				$label ? '<span class="breo-sl-btn-text">' . esc_html( $text ) . '</span>' : ''
			);
		}
		echo '</div></div>';
	}

	/** Shortcode form, e.g. [breo_social_login] on a custom page. */
	...
	private static function icon( $provider ) {
		if ( 'google' === $provider ) {
			return '<svg class="breo-sl-ico" viewBox="0 0 48 48" aria-hidden="true" focusable="false">'
		...
		return '<svg class="breo-sl-ico" viewBox="0 0 24 24" aria-hidden="true" focusable="false">'
	...
	private static function current_url() {
		...
		return esc_url_raw( remove_query_arg( 'breo_sl_error', home_url( $uri ) ) );
	}

	/** Render the ?breo_sl_error=... message as a WooCommerce-style notice. */
	public static function maybe_error() {
		if ( empty( $_GET['breo_sl_error'] ) ) {
			return;
		}
		...
		// choosing inside our error box — see BREO_SL_OAuth::messages().
		$code = sanitize_key( wp_unslash( $_GET['breo_sl_error'] ) );
		$msg  = BREO_SL_OAuth::message( $code );
		if ( $msg === '' ) {
			return;
		}
		echo '<div class="woocommerce-error breo-sl-error" role="alert">' . esc_html( $msg ) . '</div>';
	}
}
