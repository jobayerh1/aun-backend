...
delete_option( 'aun_alpha_otp' );

global $wpdb;

// Remove transients created by this plugin (prefixes: aun_alpha_otp_, _rlp_, _rli_).
$like = $wpdb->esc_like( '_transient_aun_alpha_otp_' ) . '%';
$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->options} WHERE option_name LIKE %s", $like ) ); // phpcs:ignore WordPress.DB

$like_timeout = $wpdb->esc_like( '_transient_timeout_aun_alpha_otp_' ) . '%';
