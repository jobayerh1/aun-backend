<?php
/**
 * Settings screen: Settings -> Kohthai Social Login.
 *
...
class KT_SL_Settings {

	const PAGE  = 'kohthai-social-login';
	const GROUP = 'kt_sl_group';

	public static function init() {
		add_action( 'admin_menu', array( __CLASS__, 'menu' ) );
		add_action( 'admin_init', array( __CLASS__, 'register' ) );
		add_filter( 'plugin_action_links_' . plugin_basename( KT_SL_FILE ), array( __CLASS__, 'action_link' ) );
	}

	public static function menu() {
		add_options_page( 'Kohthai Social Login', 'Kohthai Social Login', 'manage_options', self::PAGE, array( __CLASS__, 'page' ) );
	}

	public static function register() {
		register_setting( self::GROUP, KT_SL_Options::OPTION, array(
			'type'              => 'array',
			'sanitize_callback' => array( 'KT_SL_Options', 'sanitize' ),
			'default'           => KT_SL_Options::defaults(),
		) );
	}
	...
	private static function name( $key ) {
		return KT_SL_Options::OPTION . '[' . $key . ']';
	}
	...
	private static function redirect_field( $provider ) {
		$url = KT_SL_OAuth::callback_url( $provider );
	...
	private static function secret_state( $key ) {
		$stored = (string) KT_SL_Options::get( $key );
		$const  = ( 'google_client_secret' === $key ) ? 'KT_SL_GOOGLE_CLIENT_SECRET' : 'KT_SL_FACEBOOK_APP_SECRET';
	...
	private static function test_button( $provider ) {
		...
		echo '<a class="button button-secondary" href="' . esc_url( KT_SL_OAuth::test_url( $provider ) ) . '">Test ' . esc_html( $label ) . ' connection</a>';
	...
	private static function test_result() {
		$key = 'kt_sl_test_' . get_current_user_id();
		$r   = get_transient( $key );
		if ( ! is_array( $r ) ) {
			return;
		}
		...
		if ( $refused && isset( $out['code'] ) && 'kt_sl_admin' === $out['code'] ) {
	...
	public static function page() {
		...
		?>
		<div class="wrap">
			<h1>Kohthai Social Login</h1>
			<p>Sign in with Google / Facebook for WooCommerce — social login only, no sharing or comment features.</p>

			<?php self::test_result(); ?>

			<div class="card" style="max-width:900px;padding:18px 22px;margin:16px 0 24px;border-left:4px solid #654321;background:#faf7f3;">
				...
				<table class="form-table" role="presentation">
					<tr>
						...
						<td>
							...
							self::checkbox( 'at_wc_login_before', 'Above the WooCommerce login form', $o, 'Use this instead if the icons do not show up on the login side — some plugins hide or replace the email/password form, and anything inside it is hidden too.' );
							...
							?>
							<p class="description">You can also place them anywhere with the shortcode <code>[kohthai_social_login]</code>.</p>
						</td>
					</tr>
				</table>
