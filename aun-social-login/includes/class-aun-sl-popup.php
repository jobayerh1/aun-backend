 ...
 *
 * HOW IT WORKS
 *   The buttons are ordinary links to our own /?aun_sl=start flow. This script
...
class AUN_SL_Popup {
	...
	public static function render() {
		if ( is_user_logged_in() || ! AUN_SL_Options::active_providers() ) {
			return;
		}
...
(function(){
  ...
  window.addEventListener('message', function(ev){
    ...
    if (d.ok && d.redirect) {
      ...
      if (d.code) { u.searchParams.set('aun_sl_error', d.code); }
  ...
  function mountGoogle(){
    ...
    var anchors = document.querySelectorAll('a.aun-sl-google');
    if (!anchors.length) return true;   // nothing to do on this page

    var mounted = 0;
    Array.prototype.forEach.call(anchors, function(a){
      if (a.getAttribute('data-aun-g') === '1') { mounted++; return; }
      var host = document.createElement('span');
      host.className = 'aun-sl-gbtn';
      var row = a.closest ? a.closest('.aun-sl') : null;
      try {
        ...
        if (host.firstChild) {
          ...
          a.setAttribute('data-aun-g', '1');
          matchSiblings(host);
          /* Tell the stylesheet that Google is drawing a button here, so our
             hover can match theirs — see .aun-sl-native. */
          if (row) { row.classList.add('aun-sl-native'); }
          mounted++;
        } else {
  ...
  function matchSiblings(host){
    var row = host.closest ? host.closest('.aun-sl') : null;
    if (!row) return;

    function apply(){
      var h = host.offsetHeight, w = host.offsetWidth;
      if (!h) { return false; }
      row.style.setProperty('--aun-sl-size', h + 'px');
      if (w) { row.style.setProperty('--aun-sl-gw', w + 'px'); }
      return true;
    }
  ...
  function openWindow(a){
    ...
    var opened;
    try {
      opened = window.open(url, 'aun_sl_login',
  ...
  document.addEventListener('click', function(e){
    var a = e.target && e.target.closest ? e.target.closest('a.aun-sl-btn') : null;
    if (!a) return;
