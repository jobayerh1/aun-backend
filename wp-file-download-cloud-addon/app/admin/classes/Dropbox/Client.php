...
class Client
{
    ...
    function uploadFile($path, $writeMode, $inStream, $numBytes = null)
    {
        ...
        if ($numBytes === null || $numBytes > self::$AUTO_CHUNKED_UPLOAD_THRESHOLD) {
            $metadata = $this->_uploadSessionChunked($path, $writeMode, $inStream, $numBytes,
    ...
    function updateFile($path, $rev, $writeMode, $inStream, $numBytes = null)
    {
        ...
        // In all other cases, use regular upload.
        if ($numBytes === null || $numBytes > self::$AUTO_CHUNKED_UPLOAD_THRESHOLD) {
            $writeMode = array('.tag' => 'update', 'update' => $rev);
            $metadata = $this->_uploadSessionChunked($path, $writeMode, $inStream, $numBytes,
                self::$DEFAULT_CHUNK_SIZE);
        } else {
            $writeMode = WriteMode::update($rev);
    ...
    public function _uploadSessionChunked($path, $writeMode, $inStream, $numBytes, $chunkSize = -1)
    {
        ...
        $client = $this;
        $sessionId = $client->uploadSessionStart($data);
        ...
        while (!feof($inStream)) {
            ...
            $sessionId = $this->uploadSessionAppend($data, $sessionId, $byteOffset, $chunkSize);

            $byteOffset += $len;
        }


        // Finish upload session and set path for uploaded file
        $remaining = $fileSize - $byteOffset;
        $data = self::readFully($inStream, $remaining);
        $params = array(
            'mode' => $writeMode,
            'autorename' => false,
            'mute' => true // Don't send notification on this
        );

        return $this->uploadSessionFinish($data, $sessionId, $byteOffset, $path, $params);
    }

    public function uploadSessionAppend($data, $sessionId, $offset, $chunkSize, $close = false) {
        Checker::argString('data', $data);
        if (is_null($sessionId) || is_null($offset) || is_null($chunkSize)) {
            throw new Exception_BadRequest("Session ID, offset and chunk size cannot be null");
        }
        ...
        RequestUtil::runWithRetry(3,
            function() use ($data, $params) {
                $this->_uploadSessionAppend($data, $params);
            }
        );
    ...
    public function uploadSessionFinish($data, $sessionId, $offset, $path, $params)
    {
        if (is_null($sessionId) || is_null($path || is_null($offset))) {
            throw new Exception_BadRequest("Session ID, offset and path cannot be null");
        }
        ...
        $response = $this->_uploadSessionFinish($data, $queryParams);

        return json_decode($response->body, true, 10);
    }
    public function uploadSessionStart($data)
    {
        Checker::argString("data", $data);
        $response = $this->_uploadSessionStart($data);
        if ($response->statusCode === 404) {
            throw new Exception_BadResponse("Got a 404, session_id not created");
        }

        $sessionId = self::_uploadSessionCheckForSessionId($response);

        if ($sessionId === null) throw new Exception_BadResponse(
            "Got an error, session_id not created"
        );

        return $sessionId;
    }
    private static function _uploadSessionCheckForSessionId($response)
    {
    ...
    public function _uploadSessionStart($data)
    {
        $url = RequestUtil::buildUri($this->contentHost, "2/files/upload_session/start");
        $curl = $this->mkCurl($url);
        $params = array(
            'close' => false,
        );
    ...
    public function _uploadSessionAppend($data, $params)
    {
        $url = RequestUtil::buildUri($this->contentHost, "2/files/upload_session/append_v2");
        $curl = $this->mkCurl($url);
        $curl->addheader("Content-Type: application/octet-stream");
        $curl->addHeader('Dropbox-API-Arg: ' . json_encode($params));
        $curl->set(CURLOPT_POSTFIELDS, $data);
    ...
    public function _uploadSessionFinish($data, $params)
    {
        $url = RequestUtil::buildUri($this->contentHost, "2/files/upload_session/finish");
        $curl = $this->mkCurl($url);
        $curl->addHeader("Content-Type: application/octet-stream");
        $curl->addHeader('Dropbox-API-Arg: ' . json_encode($params));
        $curl->set(CURLOPT_POSTFIELDS, $data);
    ...
    {
        $params = array("path"=> $path, "recursive"=> $recursive,"include_deleted"=>false,"include_media_info"=>false);
        $response = $this->doPost(
            $this->apiHost,
        ...
        while($hasMore) {
            ...
            $metadata2 = RequestUtil::parseResponseJson($response->body);
            $metadata['entries'] = array_merge($metadata['entries'], $metadata2['entries'] );
            $hasMore = $metadata2['has_more'];
            $cursor = $metadata2['cursor'];
        }
     ...
     *
     * See <a href="https://www.dropbox.com/developers/documentation/http/documentation#files-get_temporary_link">/files/get_temporary_link</a>.
     *
     * @param string $path
     *    The Dropbox path to a file or folder (UTF-8).
     *
     * @return string
     *    A <code>string $url</code> where <code>$url</code> is a temporary direct
    ...
    function getThumbnailV2($path, $outStream, $format = "jpeg", $size = "w64h64", $mode = "strict") {
        Path::checkArgNonRoot('path', $path);
        Checker::argString('format', $format);
        Checker::argString('size', $size);
        Checker::argString('mode', $mode);

        if (!in_array($format, array("jpeg", "png"))) {
            throw new \InvalidArgumentException("Invalid 'format': ".Util::q($format));
        }
        if (!in_array($size, array("w32h32", "w64h64", "w128h128", "w256h256", "w480h320", "w640h480", "w960h640", "w1024h768", "w2048h1536"))) {
            throw new \InvalidArgumentException("Invalid 'size': ".Util::q($format));
        }
        if (!in_array($mode, array("strict", "bestfit", "fitone_bestfit"))) {
            throw new \InvalidArgumentException("Invalid 'mode': ".Util::q($mode));
        }

        $params = array("resource"=> array(".tag" => "path", "path" => $path), "size" => $size, "format" => $format, "mode" => $mode);

        $url = RequestUtil::buildUri($this->contentHost, "2/files/get_thumbnail_v2");

        $curl = $this->mkCurl($url);
        $curl->set(CURLOPT_POST, true);
        $curl->addHeader("Content-Type: application/octet-stream; charset=utf-8" );
        $curl->addHeader("Dropbox-API-Arg: ". json_encode($params));
        $metadataCatcher = new DropboxMetadataHeaderCatcher($curl->handle);
        $streamRelay = new CurlStreamRelay($curl->handle, $outStream);
    ...
    function copy($fromPath, $toPath)
    {
        ...
        if (!isset($j['metadata'])) {
            throw new Exception_BadResponse("Metadata not found");
        }
    ...
    function createFolder($path)
    {
        ...
        $json = RequestUtil::parseResponseJson($response->body);
