...
class AUN_SL_Options {

	const OPTION = 'aun_sl_options';
	...
	public static function all() {
		...
		// Seamless upgrade from the standalone aun-google-one-tap plugin: adopt the
		// Client ID it already stored so nothing has to be re-typed.
		if ( $o['google_client_id'] === '' ) {
			$legacy = get_option( 'aun_google_client_id', '' );
	...
	public static function get( $key ) {
		$map = array(
			'google_client_secret' => 'AUN_SL_GOOGLE_CLIENT_SECRET',
			'facebook_app_secret'  => 'AUN_SL_FACEBOOK_APP_SECRET',
			'google_client_id'     => 'AUN_SL_GOOGLE_CLIENT_ID',
			'facebook_app_id'      => 'AUN_SL_FACEBOOK_APP_ID',
		);
