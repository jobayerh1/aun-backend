...
class AUN_SP_Admin {
	...
	public function page_messages() {
		...
		echo '<tr><th>Better-photo request</th><td><textarea name="sms_photo" rows="2" class="large-text">' . esc_textarea( $photo ) . '</textarea><p class="description">Sent when you send a photo back on a request. <code>{reason}</code> becomes the short reason you picked (&ldquo;It shows the wrong part.&rdquo;), and the full explanation waits on the tracking page in the customer&rsquo;s own language. <code>{note}</code> is also available, but beware: your note is free text, and a single Bangla character turns one SMS into three.</p></td></tr>';
		...
		echo '<tr><th>WhatsApp chase (not an SMS)</th><td><textarea name="wa_chase" rows="3" class="large-text">' . esc_textarea( $wachase ) . '</textarea><p class="description">The message pre-written for the <strong>Chase on WhatsApp</strong> button on a request whose quote is unanswered. You still see it in WhatsApp before sending. Placeholders: <code>{name}</code> <code>{ref}</code> <code>{total}</code> <code>{track}</code>.</p></td></tr>';
