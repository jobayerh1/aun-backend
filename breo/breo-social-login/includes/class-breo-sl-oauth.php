 ...
 *   1. /?breo_sl=start&p=google      -> mint single-use state, redirect to provider
 *   2. /?breo_sl=callback&p=google   -> verify state, swap code for a token over TLS,
...
class BREO_SL_OAuth {
	...
	const STATE_PREFIX = 'breo_sl_st_';
	...
	public static function callback_url( $provider ) {
		return add_query_arg(
			array( 'breo_sl' => 'callback', 'p' => $provider ),
			self::base_url()
		);
	}
	...
	public static function start_url( $provider, $redirect = '' ) {
		$args = array( 'breo_sl' => 'start', 'p' => $provider );
		if ( $redirect !== '' ) {
	...
	public static function test_url( $provider ) {
		return wp_nonce_url(
			add_query_arg( array( 'breo_sl' => 'start', 'p' => $provider, 'test' => '1' ), self::base_url() ),
			'breo_sl_test_' . $provider
		);
	}
	...
	public static function maybe_handle() {
		if ( empty( $_GET['breo_sl'] ) ) {
			return;
		}
		$action   = sanitize_key( wp_unslash( $_GET['breo_sl'] ) );
		...
		if ( 'start' === $action ) {
			...
			if ( $is_test ) {
				...
				if ( ! current_user_can( 'manage_options' ) || ! wp_verify_nonce( $nonce, 'breo_sl_test_' . $provider ) ) {
	...
	private static function authorize_url( $provider, $state ) {
		if ( 'google' === $provider ) {
			return add_query_arg(
				array(
					'client_id'     => rawurlencode( BREO_SL_Options::get( 'google_client_id' ) ),
		...
		return add_query_arg(
			array(
				'client_id'     => rawurlencode( BREO_SL_Options::get( 'facebook_app_id' ) ),
	...
	public static function authenticate( $provider, $profile ) {
		...
		if ( BREO_SL_Options::get( 'use_avatar' ) && ! empty( $profile['avatar'] ) ) {
			BREO_SL_Avatar::store( $user_id, $profile['avatar'] );
		}
	...
	private static function exchange_code( $provider, $code ) {
		if ( 'google' === $provider ) {
			...
			$body = array(
				...
				'client_secret' => BREO_SL_Options::get( 'google_client_secret' ),
			...
			$body = array(
				'code'          => $code,
				'client_id'     => BREO_SL_Options::get( 'facebook_app_id' ),
				'client_secret' => BREO_SL_Options::get( 'facebook_app_secret' ),
	...
	private static function fetch_profile( $provider, $token ) {
		...
		$secret = BREO_SL_Options::get( 'facebook_app_secret' );
	...
	private static function finish_test( $provider, $ok, $error = '', $profile = array() ) {
		...
		if ( $ok && $profile ) {
			$result['profile'] = array(
				...
				'avatar'   => ! empty( $profile['avatar'] ) ? BREO_SL_Avatar::sanitize_url( $profile['avatar'] ) : '',
			);
			$result['outcome'] = self::decide( $provider, $profile );
		}
		set_transient( 'breo_sl_test_' . get_current_user_id(), $result, 5 * MINUTE_IN_SECONDS );

		wp_safe_redirect( admin_url( 'options-general.php?page=breo-social-login&breo_sl_test=1' ) );
		exit;
	}
	...
	private static function login_or_register( $provider, $profile ) {
		...
		$meta_key = 'breo_sl_' . $provider . '_id';
	...
	private static function decide( $provider, $profile ) {
		...
		if ( count( $linked ) > 1 ) {
			...
			return self::refuse( 'breo_sl_dup', 'We found more than one account for this profile. Please contact support.' );
		}
		if ( count( $linked ) === 1 ) {
			$uid = (int) $linked[0];
			if ( self::is_blocked_admin( $uid ) ) {
				return self::refuse( 'breo_sl_admin', 'For security, staff accounts must sign in with a password rather than a social account.' );
			}
		...
		if ( $email === '' || ! is_email( $email ) ) {
			return self::refuse( 'breo_sl_noemail', 'Your social profile did not share an email address, so we cannot create your account. Please register with your email or phone number instead.' );
		}

		$existing = get_user_by( 'email', $email );
		if ( $existing ) {
			if ( ! BREO_SL_Options::get( 'link_by_email' ) || empty( $profile['email_verified'] ) ) {
				return self::refuse( 'breo_sl_exists', 'An account already exists with this email address. Please sign in with your password to continue.' );
			}
		...
		if ( ! BREO_SL_Options::get( 'allow_register' ) ) {
			return self::refuse( 'breo_sl_noreg', 'New account creation is currently disabled. Please contact support.' );
		}
	...
	private static function is_blocked_admin( $user_id ) {
		return BREO_SL_Options::get( 'block_admins' ) && user_can( $user_id, 'edit_others_posts' );
	}
	...
	private static function create_user( $provider, $profile, $meta_key ) {
		...
		update_user_meta( $user_id, 'breo_sl_registered_via', $provider );
		...
		if ( BREO_SL_Options::get( 'notify_admin' ) ) {
	 ...
	 * printed on the account page straight from ?breo_sl_error=. If that carried the
	...
	public static function messages() {
		return array(
			...
			'breo_sl_dup'     => 'We found more than one account for this profile. Please contact support.',
			'breo_sl_admin'   => 'For security, staff accounts must sign in with a password rather than a social account.',
			'breo_sl_noemail' => 'Your social profile did not share an email address, so we cannot create your account. Please register with your email or phone number instead.',
			'breo_sl_exists'  => 'An account already exists with this email address. Please sign in with your password to continue.',
			'breo_sl_noreg'   => 'New account creation is currently disabled. Please contact support.',
		);
	}
	...
	private static function fail( $code ) {
		...
		wp_safe_redirect( add_query_arg( 'breo_sl_error', $code, self::default_redirect() ) );
		exit;
	}
...
(function(){
  ...
  var fallback = <?php echo wp_json_encode( $redirect !== '' ? $redirect : add_query_arg( 'breo_sl_error', $code, self::default_redirect() ) ); ?>;
  try {
  ...
  window.location.replace(fallback);
	...
	public static function rate_ok() {
		...
		$key = 'breo_sl_rl_' . hash( 'sha256', $ip );
	...
	private static function log( $msg ) {
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			error_log( 'Breo Social Login: ' . $msg );  // never contains tokens
		}
	}
}
