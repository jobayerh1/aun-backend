 ...
 */
class WpfdAddonModelCloudcategory extends Model
{
    /**
    ...
    /**
     * WpfdAddonModelCloudcategory constructor.
     */
    ...
    public function addCategory($title, $parent = 0, $position = 'end')
    {
        ...
        $cloudConfigs      = WpfdAddonHelper::getAllCloudConfigs();
        if ($parent === 0) {
            $googleBaseFolder  = $cloudConfigs['googleBaseFolder'];
        } else {
            $googleBaseFolder = WpfdAddonHelper::getGoogleDriveIdByTermId($parent);
        }
        $cloud = $this->addFolderOnGoogleDrive($title, $googleBaseFolder);
        ...
        update_term_meta($inserted['term_id'], 'wpfd_drive_type', 'googleDrive');
     ...
     */
    public function addFolderOnGoogleDrive($title, $parentId)
    {
        return $this->googleCate->createFolder($title, $parentId);
    }
     ...
     * @return boolean
     */
    public function addCategoryFromGoogleDrive($title, $cloudId, $parent)
    {
    ...
    public function changeParent($termId, $parent)
    {
        ...
        foreach ($cloudParams['googledrive'] as $key => $val) {
    ...
    public function deleteCategory($id_category)
    {
        ...
        $result = false;
        if (!empty($cloudId) && $cloudType === 'googleDrive') {
            if ($this->googleCate->delete($cloudId)) {
    ...
    public function changeOrder($pk)
    {
        ...
        $params   = WpfdAddonHelper::getAllCloudConfigs();
        if ($params['googleCredentials']) {
            $itemInfo = $category->getCategory($pk);
            //$parentInfo=$category->getCategory($itemInfo->parent);
            if (WpfdAddonHelper::getGoogleDriveIdByTermId($itemInfo->parent)) {
                $this->googleCate->moveFile(
                    WpfdAddonHelper::getGoogleDriveIdByTermId($pk),
                    WpfdAddonHelper::getGoogleDriveIdByTermId($itemInfo->parent)
                );
            } else {
                $this->googleCate->moveFile(
                    WpfdAddonHelper::getGoogleDriveIdByTermId($pk),
                    $params['googleBaseFolder']
                );
            }
        }
    }
    ...
    public function changeCategoryName($cloudId, $title)
    {
        ...
        return $this->googleCate->changeFilename($cloudId, $title);
    }
    ...
    public function deleteCategoryWhenDisconnect()
    {
        ...
        $results = $wpdb->get_results(
            $wpdb->prepare('SELECT term_id FROM ' . $wpdb->termmeta . ' WHERE meta_value = %s GROUP BY term_id', 'googleDrive')
        );
