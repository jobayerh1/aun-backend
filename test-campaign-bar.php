<?php
/**
 * Regression harness for aun-campaign-bar.
 *
 *   cd ~/wp-local
...
if ( ! class_exists( 'AUN_Campaign_Notice_Bar' ) ) {
	fwrite( STDERR, "aun-campaign-bar is not active on this bench.\n" );
	exit( 1 );
}
...
function priv( $method, $args = array() ) {
	$m = new ReflectionMethod( 'AUN_Campaign_Notice_Bar', $method );
...
$opts = get_option( AUN_Campaign_Notice_Bar::OPTION_KEY );
...
set_window( -3600, 3600 );
$in = do_shortcode( '[aun_campaign_notice]' );
t( 'inside the window: message present', false !== strpos( $in, 'CAMPAIGN-MARKER' ), true );
t( 'inside the window: NOT hidden', false === strpos( $in, 'class="aun-cb-top" data-start' ) ? null : ( false === strpos( $in, ' hidden>' ) ), true );
t( 'inside the window: carries the timer script', false !== strpos( $in, 'aun-cb-top' ), true );

set_window( 3600, 7200 );
$pre = do_shortcode( '[aun_campaign_notice]' );
...
$post = do_shortcode( '[aun_campaign_notice]' );
t( 'after it closes: no message', false === strpos( $post, 'CAMPAIGN-MARKER' ), true );
t( 'after it closes: nothing visible', false === strpos( $post, 'aun-cb-msg' ), true );

set_window( -3600, 3600, array( 'enabled' => '0' ) );
$off = do_shortcode( '[aun_campaign_notice]' );
t( 'disabled: no message', false === strpos( $off, 'CAMPAIGN-MARKER' ), true );
t( 'disabled: nothing visible', false === strpos( $off, 'aun-cb-msg' ), true );

set_window( null, null );
$open = do_shortcode( '[aun_campaign_notice]' );
t( 'open-ended campaign: message shown', false !== strpos( $open, 'CAMPAIGN-MARKER' ), true );
t( 'open-ended campaign: still wrapped, for the dismiss button', false !== strpos( $open, 'class="aun-cb-x"' ), true );

// Nothing to time and nothing to close: emit plain markup, no wrapper, no script.
set_window( null, null, array( 'allow_dismiss' => '0' ) );
$plain = do_shortcode( '[aun_campaign_notice]' );
t( 'open-ended + not dismissible: no wrapper at all', false === strpos( $plain, 'aun-cb-top' ), true );
...
set_window( 3600, 7200 );
$html = do_shortcode( '[aun_campaign_notice]' );
$w    = AUN_Campaign_Notice_Bar::window_ts();
...
$flag = new ReflectionProperty( 'AUN_Campaign_Notice_Bar', 'top_printed' );
$flag->setAccessible( true );
$flag->setValue( null, false );

set_window( -3600, 3600 );
$twice = do_shortcode( '[aun_campaign_notice][aun_campaign_notice]' );
t( 'message rendered by both shortcodes', substr_count( $twice, 'CAMPAIGN-MARKER' ), 2 );
t( 'style block printed only once', substr_count( $twice, 'aun-cb-bar-empty{display:none' ), 1 );
...
preg_match_all( '/id="(aun-cb-\d+)"/', $twice, $ids );
...
$flag->setValue( null, false );
$autop = wpautop( do_shortcode( '[aun_campaign_notice]' ) );
t( 'survives wpautop: the id is still on the notice', 1, preg_match( '/id="aun-cb-\d+"/', $autop ) );
...
$w = AUN_Campaign_Notice_Bar::window_ts();
t( 'a purge is scheduled at the start', wp_next_scheduled( 'aun_cb_boundary', array( 'start' ) ), $w['start'] );
t( 'a purge is scheduled at the end', wp_next_scheduled( 'aun_cb_boundary', array( 'end' ) ), $w['end'] );
...
t( 'a 3-minute campaign still gets BOTH purges',
	array( wp_next_scheduled( 'aun_cb_boundary', array( 'start' ) ), wp_next_scheduled( 'aun_cb_boundary', array( 'end' ) ) ),
	array( $w['start'], $w['end'] ) );
...
$fired = 0;
add_action( 'aun_cb_cache_purged', function () use ( &$fired ) { $fired++; } );

set_window( -7200, -3600 );
AUN_Campaign_Notice_Bar::reschedule_boundaries();
$fired = 0;
AUN_Campaign_Notice_Bar::catch_up_boundary();
...
foreach ( array( 'off', 'mobile', 'always' ) as $mode ) {
	$saved = AUN_Campaign_Notice_Bar::sanitize_options( array( '_form' => '1', 'hide_empty_topbar' => $mode, 'enabled' => '1' ) );
...
$saved = AUN_Campaign_Notice_Bar::sanitize_options( array( '_form' => '1', 'hide_empty_topbar' => '<script>', 'enabled' => '1' ) );
...
$after = AUN_Campaign_Notice_Bar::sanitize_options( array( 'message' => 'ONLY THIS CHANGED' ) );
...
$form = AUN_Campaign_Notice_Bar::sanitize_options( array( '_form' => '1', 'message' => 'x' ) );
...
$tzbad = AUN_Campaign_Notice_Bar::sanitize_options( array( 'timezone' => 'Mars/Olympus' ) );
t( 'an unknown timezone falls back to Asia/Dhaka', $tzbad['timezone'], 'Asia/Dhaka' );
$tzok = AUN_Campaign_Notice_Bar::sanitize_options( array( 'timezone' => 'Asia/Kolkata' ) );
t( 'a real timezone is accepted', $tzok['timezone'], 'Asia/Kolkata' );

t( 'non-array input is ignored', AUN_Campaign_Notice_Bar::sanitize_options( 'nonsense' ), AUN_Campaign_Notice_Bar::get_options() );

/* ------------------------------------------------ H. the "Save ৳X" figure */

echo "\nH. saving_for() on a variable product\n";

$vid = wc_get_products( array( 'limit' => 1, 'return' => 'ids', 'type' => 'variable', 'name' => 'harness-variable-projector' ) );
$vid = $vid ? $vid[0] : 0;
if ( ! $vid ) {
	$parent = new WC_Product_Variable();
	$parent->set_name( 'Harness Variable Projector' );
	$parent->set_slug( 'harness-variable-projector' );
...
$cd = do_shortcode( '[aun_campaign_notice]' );
$w  = AUN_Campaign_Notice_Bar::window_ts();

t( 'the tag is replaced', false === strpos( $cd, '{countdown}' ), true );
t( 'a countdown element is rendered', false !== strpos( $cd, 'class="aun-cb-cd"' ), true );
...
t( 'the server pre-renders a value for no-JS visitors',
	1, preg_match( '/class="aun-cb-cd"[^>]*>\d+d \d\d:\d\d:\d\d</', $cd ) );
...
$nocd = do_shortcode( '[aun_campaign_notice]' );
...
set_window( -60, 3600 );
$d1 = do_shortcode( '[aun_campaign_notice]' );
// Match the BUTTON, not the .aun-cb-x rule that also lives in the style block.
t( 'a close button is rendered', false !== strpos( $d1, 'class="aun-cb-x"' ), true );
...
$d2 = do_shortcode( '[aun_campaign_notice]' );
t( 'the button disappears when switched off', false === strpos( $d2, 'class="aun-cb-x"' ), true );
...
preg_match( '/data-sig="([0-9a-f]+)"/', do_shortcode( '[aun_campaign_notice]' ), $s2 );
t( 'a new message means a new signature', $s1[1] !== $s2[1], true );

set_window( -60, 7200, array( 'message' => 'CAMPAIGN-MARKER' ) );
preg_match( '/data-sig="([0-9a-f]+)"/', do_shortcode( '[aun_campaign_notice]' ), $s3 );
set_window( -60, 3600, array( 'message' => 'CAMPAIGN-MARKER' ) );
preg_match( '/data-sig="([0-9a-f]+)"/', do_shortcode( '[aun_campaign_notice]' ), $s4 );
t( 'new dates mean a new signature', $s3[1] !== $s4[1], true );

$sanit = AUN_Campaign_Notice_Bar::sanitize_options( array( 'dismiss_days' => '-5' ) );
t( 'dismiss_days -5 clamps to 1, not 5', $sanit['dismiss_days'], '1' );
$sanit = AUN_Campaign_Notice_Bar::sanitize_options( array( 'dismiss_days' => '9999' ) );
...
set_window( 3600, 7200 );
t( 'before the start reads as scheduled', AUN_Campaign_Notice_Bar::campaign_state()['key'], 'scheduled' );
set_window( -3600, 3600 );
t( 'inside the window reads as live', AUN_Campaign_Notice_Bar::campaign_state()['key'], 'live' );
set_window( -7200, -3600 );
$ended = AUN_Campaign_Notice_Bar::campaign_state();
...
set_window( -3600, 3600, array( 'enabled' => '0' ) );
t( 'disabled reads as off', AUN_Campaign_Notice_Bar::campaign_state()['key'], 'off' );
set_window( -3600, 3600, array( 'message' => '' ) );
$empty = AUN_Campaign_Notice_Bar::campaign_state();
...
set_window( null, null );
t( 'open-ended is live', AUN_Campaign_Notice_Bar::campaign_state()['key'], 'live' );
t( '  ...and says there is no end date', false !== strpos( AUN_Campaign_Notice_Bar::campaign_state()['detail'], 'No end date' ), true );
...
function head_css() {
	ob_start();
	AUN_Campaign_Notice_Bar::maybe_hide_topbar_css();
	return ob_get_clean();
}
...
foreach ( array(
	...
	t( "$label: top_is_showing() is false", AUN_Campaign_Notice_Bar::top_is_showing(), false );
	$css = head_css();
	...
	t( "  $label: the shortcode emits nothing at all", trim( do_shortcode( '[aun_campaign_notice]' ) ), '' );
}

// A live campaign fills the bar, so it must NOT be hidden.
set_window( -3600, 3600 );
t( 'live campaign: top_is_showing() is true', AUN_Campaign_Notice_Bar::top_is_showing(), true );
...
t( "legacy '1' migrates to mobile", AUN_Campaign_Notice_Bar::hide_mode( '1' ), 'mobile' );
t( "legacy '0' migrates to off", AUN_Campaign_Notice_Bar::hide_mode( '0' ), 'off' );
t( 'a nonsense value falls back to mobile', AUN_Campaign_Notice_Bar::hide_mode( 'banana' ), 'mobile' );

update_option( AUN_Campaign_Notice_Bar::OPTION_KEY, array_merge(
	AUN_Campaign_Notice_Bar::defaults(), array( 'hide_empty_topbar' => '1' ) ) );
t( 'a stored legacy value reads back as a mode',
	AUN_Campaign_Notice_Bar::get_options()['hide_empty_topbar'], 'mobile' );

// The selector and breakpoint are filterable so a theme update is a one-liner.
add_filter( 'aun_cb_topbar_selector', function () { return '.my-custom-bar'; } );
add_filter( 'aun_cb_topbar_breakpoint', function () { return 600; } );
set_window( -7200, -3600 );
$css = head_css();
...
remove_all_filters( 'aun_cb_topbar_selector' );
remove_all_filters( 'aun_cb_topbar_breakpoint' );

// The notice script only ever flips that one style element now.
set_window( -3600, 3600 );
$live = do_shortcode( '[aun_campaign_notice]' );
t( 'the script targets the head style element', false !== strpos( $live, "getElementById('aun-cb-hidebar')" ), true );
...
$tpls = AUN_Campaign_Notice_Bar::templates();
...
$valid = array_keys( AUN_Campaign_Notice_Bar::defaults() );
...
foreach ( array( 'countdown', 'lastchance' ) as $key ) {
	...
	$out = do_shortcode( '[aun_campaign_notice]' );
	t( "'$key' with an end date: renders a live timer", false !== strpos( $out, 'class="aun-cb-cd"' ), true );
	...
	$none = do_shortcode( '[aun_campaign_notice]' );
	...
	preg_match( '/<span class="aun-cb-msg">(.*?)<\/span><button/s', $none, $mm );
...
$prod = do_shortcode( '[aun_campaign_product_notice]' );
t( 'the product notice renders a timer', false !== strpos( $prod, 'class="aun-cb-cd"' ), true );
t( '  ...with no raw tag', false === strpos( $prod, '{countdown}' ), true );

// The Bangla halves must strip cleanly too — the danda ends a sentence there.
set_window( null, null, $tpls['countdown']['fields'] );
$bn = priv( 'inject_countdown', array( $tpls['countdown']['fields']['message_bn'], 0, true ) );
t( 'Bangla: the tag is removed with no end date', false === strpos( $bn, '{countdown}' ), true );
t( '  ...and the "শেষ হতে বাকি" clause goes with it', false === strpos( $bn, 'শেষ হতে বাকি' ), true );
t( '  ...while the Bangla call to action survives', false !== strpos( $bn, 'এখনই কিনুন' ), true );

/* ------------------------------------- N. campaign vs scheduled sale */

echo "\nN. a campaign AND a product sale at the same time\n";

// A real product on a scheduled sale with an end date — the only kind that trips
// the "Flash Sale Active!" magnet.
$sale_id = wc_get_products( array( 'limit' => 1, 'return' => 'ids', 'type' => 'simple', 'status' => 'publish' ) );
$sale_id = $sale_id ? $sale_id[0] : 0;
if ( ! $sale_id ) {
	$np = new WC_Product_Simple();
	$np->set_name( 'Harness Sale Projector' );
	$np->set_status( 'publish' );
	$sale_id = $np->save();
}
$sp = wc_get_product( $sale_id );
$sp->set_regular_price( '30000' );
$sp->set_sale_price( '25000' );
$sp->set_date_on_sale_from( time() - HOUR_IN_SECONDS );
$sp->set_date_on_sale_to( time() + 3 * DAY_IN_SECONDS );
$sp->save();
AUN_Campaign_Notice_Bar::flush_sale_cache();

/** Set a live campaign with smart sales ON, at the given priority. */
function both_running( $priority ) {
	$tz  = new DateTimeZone( 'Asia/Dhaka' );
	$fmt = function ( $off ) use ( $tz ) { $d = new DateTime( '@' . ( time() + $off ) ); $d->setTimezone( $tz ); return $d->format( 'Y-m-d H:i' ); };
	update_option( AUN_Campaign_Notice_Bar::OPTION_KEY, array_merge(
		AUN_Campaign_Notice_Bar::defaults(),
		array(
			'enabled'            => '1',
			...
			'timezone'           => 'Asia/Dhaka',
			'start'              => $fmt( -3600 ),
			'end'                => $fmt( 3 * DAY_IN_SECONDS ),
			'message'            => 'EID-CAMPAIGN',
			'smart_sale_global_top_message' => 'FLASH-SALE-MAGNET',
		)
	) );
	AUN_Campaign_Notice_Bar::flush_sale_cache();
}

t( 'the magnet really is triggered by this product',
	priv( 'any_scheduled_sale_active' ), true );

both_running( 'campaign' );
$out = do_shortcode( '[aun_campaign_notice]' );
t( 'campaign-first: the campaign shows', false !== strpos( $out, 'EID-CAMPAIGN' ), true );
t( '  ...and the sale magnet is held back', false === strpos( $out, 'FLASH-SALE-MAGNET' ), true );

both_running( 'sale' );
$out = do_shortcode( '[aun_campaign_notice]' );
t( 'sale-first: the magnet shows instead', false !== strpos( $out, 'FLASH-SALE-MAGNET' ), true );
t( '  ...and the campaign is suppressed', false === strpos( $out, 'EID-CAMPAIGN' ), true );

// The magnet must still fill the bar when there is no campaign to show.
both_running( 'campaign' );
$o = get_option( AUN_Campaign_Notice_Bar::OPTION_KEY );
$o['enabled'] = '0';
update_option( AUN_Campaign_Notice_Bar::OPTION_KEY, $o );
AUN_Campaign_Notice_Bar::flush_sale_cache();
t( 'no campaign: the magnet fills the bar',
	false !== strpos( do_shortcode( '[aun_campaign_notice]' ), 'FLASH-SALE-MAGNET' ), true );

// An empty message on the winning side must not blank the bar.
both_running( 'campaign' );
$o = get_option( AUN_Campaign_Notice_Bar::OPTION_KEY );
$o['message'] = '';
$o['message_bn'] = '';
update_option( AUN_Campaign_Notice_Bar::OPTION_KEY, $o );
AUN_Campaign_Notice_Bar::flush_sale_cache();
t( 'campaign-first with an empty campaign message falls through to the magnet',
	false !== strpos( do_shortcode( '[aun_campaign_notice]' ), 'FLASH-SALE-MAGNET' ), true );

// Whichever way round, something is showing — so the top bar must stay open.
both_running( 'campaign' );
t( 'the top bar is not collapsed while either is showing',
	AUN_Campaign_Notice_Bar::top_is_showing(), true );

$d = AUN_Campaign_Notice_Bar::defaults();
t( 'the default is campaign-first', $d['notice_priority'], 'campaign' );
$sn = AUN_Campaign_Notice_Bar::sanitize_options( array( '_form' => '1', 'notice_priority' => 'sale' ) );
t( "'sale' survives a Save", $sn['notice_priority'], 'sale' );
$sn = AUN_Campaign_Notice_Bar::sanitize_options( array( '_form' => '1', 'notice_priority' => 'nonsense' ) );
t( 'a junk priority falls back to campaign', $sn['notice_priority'], 'campaign' );

// Tidy up so the sale does not leak into other runs of this file.
$sp->set_sale_price( '' );
$sp->set_date_on_sale_to( null );
$sp->set_date_on_sale_from( null );
$sp->save();
AUN_Campaign_Notice_Bar::flush_sale_cache();
...
delete_option( 'aun_cb_boundaries' );
wp_clear_scheduled_hook( 'aun_cb_boundary', array( 'start' ) );
wp_clear_scheduled_hook( 'aun_cb_boundary', array( 'end' ) );
