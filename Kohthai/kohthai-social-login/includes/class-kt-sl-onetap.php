 ...
 *
 * Merged in from the standalone kohthai-google-one-tap plugin. The important change is
 ...
 * KT_SL_OAuth::authenticate(), so it inherits *every* guard — provider-id linking,
 ...
 * class-kt-sl-popup.php. A custom anchor calling prompt() cannot produce it.
 *
...
class KT_SL_OneTap {
	...
	public static function init() {
		if ( ! KT_SL_Options::provider_ready( 'google' ) ) {
			return;
		}
		add_action( 'wp_footer', array( __CLASS__, 'render' ), 99 );
		add_action( 'wp_ajax_nopriv_kt_sl_onetap', array( __CLASS__, 'ajax_login' ) );
		add_action( 'wp_ajax_kt_sl_onetap', array( __CLASS__, 'ajax_login' ) );
		add_action( 'wp_ajax_nopriv_kt_sl_nonce', array( __CLASS__, 'ajax_nonce' ) );
		add_action( 'wp_ajax_kt_sl_nonce', array( __CLASS__, 'ajax_nonce' ) );
	}

	/** Hands out a fresh nonce so cached HTML never carries a stale one. */
	public static function ajax_nonce() {
		wp_send_json_success( array( 'nonce' => wp_create_nonce( 'kt_sl_onetap' ) ) );
	}
	...
	public static function render() {
		...
		$auto = (bool) KT_SL_Options::get( 'onetap_enabled' );
		$btn  = (bool) KT_SL_Options::get( 'js_flow' );
		if ( ! $auto && ! $btn ) {
		...
		function ktSlOneTap( response ) {
			if ( ! response || ! response.credential ) { return; }
			var ajax = <?php echo wp_json_encode( $ajax ); ?>;
			fetch( ajax + '?action=kt_sl_nonce', { credentials: 'same-origin' } )
				.then( function ( r ) { return r.json(); } )
				.then( function ( n ) {
					var body = new URLSearchParams();
					body.append( 'action', 'kt_sl_onetap' );
	...
	public static function ajax_login() {
		...
		if ( ! check_ajax_referer( 'kt_sl_onetap', 'security', false ) ) {
		...
		$user_id = KT_SL_OAuth::authenticate( 'google', $profile );
	...
	private static function verify_credential( $credential ) {
		...
		if ( is_wp_error( $res ) ) {
			return new WP_Error( 'kt_sl_net', 'Could not reach Google to verify the sign-in.' );
		}
		if ( 200 !== (int) wp_remote_retrieve_response_code( $res ) ) {
			return new WP_Error( 'kt_sl_bad_token', 'Google rejected the sign-in. Please try again.' );
		}
		$c = json_decode( wp_remote_retrieve_body( $res ), true );
		if ( ! is_array( $c ) ) {
			return new WP_Error( 'kt_sl_bad_token', 'Unreadable response from Google.' );
		}
		...
		if ( empty( $c['aud'] ) || ! hash_equals( (string) $client_id, (string) $c['aud'] ) ) {
			return new WP_Error( 'kt_sl_aud', 'This sign-in was not issued for this website.' );
		}
		// 2. Issuer must be Google.
		if ( empty( $c['iss'] ) || ! in_array( $c['iss'], array( 'accounts.google.com', 'https://accounts.google.com' ), true ) ) {
			return new WP_Error( 'kt_sl_iss', 'Unexpected token issuer.' );
		}
		// 3. Not expired.
		if ( empty( $c['exp'] ) || (int) $c['exp'] < time() ) {
			return new WP_Error( 'kt_sl_exp', 'That sign-in has expired. Please try again.' );
		}
		...
		if ( ! in_array( $verified, array( true, 'true', 1, '1' ), true ) ) {
			return new WP_Error( 'kt_sl_unverified', 'Your Google email address is not verified.' );
		}
		if ( empty( $c['sub'] ) || empty( $c['email'] ) || ! is_email( $c['email'] ) ) {
			return new WP_Error( 'kt_sl_payload', 'Incomplete Google profile.' );
		}
		return $c;
	}
