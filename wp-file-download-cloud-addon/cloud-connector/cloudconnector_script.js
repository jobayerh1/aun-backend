jQuery(document).ready(function ($) {
    $(".ggd-automatic-connect, .ggtd-automatic-connect, .onedrive-automatic-connect, .onedrive-business-automatic-connect, .dropbox-automatic-connect").on('click', function (e) {
        e.preventDefault();
        var data_link = $(this).data('link');
        var data_network = $(this).data('network');
        var extension = 'wp-file-download-addon.zip';
        var google_connect = false;
        var google_type = 'google_drive';
        var name_connect = $(this).attr('name');

        if (name_connect === 'wpfd_google-team-drive-connect' || name_connect === 'wpfd_ggdrive-connect') {
            google_connect = true;
            if (name_connect === 'wpfd_google-team-drive-connect') {
                google_type = 'google_team_drive';
            }
        }

        if (google_connect) {
            $.ajax({
                url: cloudconnector_var.ajaxurl + '?action=cloudconnector_google_type',
                method: 'POST',
                dataType: 'json',
                data: {
    ...
    $(".ggd-automatic-disconnect, .ggtd-automatic-disconnect, .onedrive-automatic-disconnect, .onedrive-business-automatic-disconnect, .dropbox-automatic-disconnect").on('click', function (e) {
        e.preventDefault();
        var $that = $(e.target);
    ...
    $('.google-team-drive-mode-radio-field .ju-radiobox').on('click', function () {
        var val = $(this).val();
        if (val === 'automatic') {
            ...
            if ($('#wpfd-btn-automaticdisconnect-ggtd').hasClass("ju-visibled")) {
                $('#wpfd-btn-automaticdisconnect-ggtd').show();
            } else {
                $('#wpfd-btn-automaticdisconnect-ggtd').hide();
            }

            if ($('#wpfd-btn-automaticconnect-ggtd').hasClass("ju-visibled")) {
                $('#wpfd-btn-automaticconnect-ggtd').show();
            } else {
                $('#wpfd-btn-automaticconnect-ggtd').hide();
            }
        } else {
            $('input[name="googleTeamDriveClientId"]').parents('.ju-settings-option').show();
            $('input[name="googleTeamDriveClientSecret"]').parents('.ju-settings-option').show();
            $('#ggtd_setup .links').show();
            $('#ggtdconnect').show();
            $('#ggtd_disconnect').show();
            $('#wpfd-btn-automaticconnect-ggtd').hide();
            $('#wpfd-btn-automaticdisconnect-ggtd').hide();
        ...
        $.ajax({
            url: cloudconnector_var.ajaxurl + '?action=cloudconnector_google_team_drive_changemode',
            method: 'POST',
            dataType: 'json',
            data: {
                value: val,
