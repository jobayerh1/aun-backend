<?php
/**
 * Regression harness for the breo-social-login plugin.
 *
 * Run against the local bench:
 *   cd ~/wp-local
 *   php wp-cli.phar eval-file "<path>/test-social-login.php" --skip-themes
 *
 * The plugin must be present in wp-content/plugins/breo-social-login and active.
 *
 ...
 *      crafted ?breo_sl_error= must render nothing.
 */

if ( ! class_exists( 'BREO_SL_OAuth' ) ) {
	fwrite( STDERR, "breo-social-login is not active on this bench.\n" );
	exit( 1 );
}
...
function set_opts( array $over ) {
	update_option( 'breo_sl_options', array_merge( BREO_SL_Options::defaults(), $over ) );
}
...
function decide( $profile, $provider = 'google' ) {
	return call_private( 'BREO_SL_OAuth', 'decide', array( $provider, $profile ) );
}
...
delete_user_meta( $cust->ID, 'breo_sl_google_id' );
delete_user_meta( $admin_id, 'breo_sl_google_id' );
...
t( '  ...and the refusal code is the "use your password" one',
	decide( profile( array( 'email_verified' => false ) ) )['code'], 'breo_sl_exists' );
...
t( 'allow_register off -> unknown email refuses',
	decide( profile( array( 'email' => 'brand-new@example.com' ) ) )['code'], 'breo_sl_noreg' );

set_opts( $base );
t( 'no email from provider -> refuse',
	decide( profile( array( 'email' => '' ) ) )['code'], 'breo_sl_noemail' );

t( 'malformed email -> refuse',
	decide( profile( array( 'email' => 'not-an-email' ) ) )['code'], 'breo_sl_noemail' );

// Now link the profile id to the customer: the link must win over everything.
update_user_meta( $cust->ID, 'breo_sl_google_id', 'g-100' );
...
// Administrator block.
update_user_meta( $admin_id, 'breo_sl_google_id', 'g-admin' );
t( 'linked ADMIN account -> refused while block_admins is on',
	decide( profile( array( 'id' => 'g-admin' ) ) )['code'], 'breo_sl_admin' );
...
update_user_meta( $cust->ID, 'breo_sl_google_id', 'g-dup' );
update_user_meta( $admin_id, 'breo_sl_google_id', 'g-dup' );
t( 'two accounts share one provider id -> refuse, never guess',
	decide( profile( array( 'id' => 'g-dup' ) ) )['code'], 'breo_sl_dup' );
delete_user_meta( $cust->ID, 'breo_sl_google_id' );
delete_user_meta( $admin_id, 'breo_sl_google_id' );
...
foreach ( $avatar_cases as $c ) {
	list( $url, $ok ) = $c;
	$got = BREO_SL_Avatar::sanitize_url( $url );
...
t( 'verify_credential() still exists (harness mirrors it)',
	method_exists( 'BREO_SL_OneTap', 'verify_credential' ) || (bool) ( new ReflectionMethod( 'BREO_SL_OneTap', 'verify_credential' ) ), true );

function check_claims( $c ) {
	$client_id = BREO_SL_Options::get( 'google_client_id' );
...
$messages = BREO_SL_OAuth::messages();
...
$src = file_get_contents( WP_PLUGIN_DIR . '/breo-social-login/includes/class-breo-sl-oauth.php' );
...
foreach ( array( 'breo_sl_dup', 'breo_sl_admin', 'breo_sl_noemail', 'breo_sl_exists', 'breo_sl_noreg' ) as $code ) {
	t( "decide() code $code is mapped", BREO_SL_OAuth::message( $code ) !== '', true );
}

t( 'an unknown code renders nothing (no attacker-chosen wording)',
	BREO_SL_OAuth::message( 'your_account_is_locked_call_01700000000' ), '' );
t( 'a raw sentence as the code renders nothing',
	BREO_SL_OAuth::message( 'Your account is locked. Call support.' ), '' );
...
function current_url_for( $request_uri ) {
	...
	return call_private( 'BREO_SL_UI', 'current_url' );
}
...
t( 'a stale error notice is not carried onto the next page',
	current_url_for( '/cart/?breo_sl_error=expired' ), $home . '/cart/' );
...
$defaults = BREO_SL_Options::defaults();
foreach ( array( 'at_wc_login', 'at_wc_login_before', 'at_wc_register', 'at_wc_checkout', 'at_wc_cart', 'at_wp_login' ) as $k ) {
	...
	$saved = BREO_SL_Options::sanitize( array( $k => '1' ) );
...
foreach ( array( 'google', 'facebook' ) as $p ) {
	t( "$p callback URI is identical without TranslatePress",
		BREO_SL_OAuth::callback_url( $p ), $home . '/?breo_sl=callback&p=' . $p );
}
...
t( 'the URI shown to the admin is the one used in the flow',
	BREO_SL_OAuth::callback_url( 'google' ), $home . '/?breo_sl=callback&p=google' );

// https must survive: get_option('home') is raw, so the scheme has to be reapplied.
t( 'the callback URI keeps the site scheme',
	0 === strpos( BREO_SL_OAuth::callback_url( 'google' ), parse_url( $home, PHP_URL_SCHEME ) . '://' ), true );
...
t( 'js_flow defaults ON', BREO_SL_Options::defaults()['js_flow'], 1 );
t( 'js_flow survives a Save', BREO_SL_Options::sanitize( array( 'js_flow' => '1' ) )['js_flow'], 1 );
t( 'js_flow can be turned off', BREO_SL_Options::sanitize( array() )['js_flow'], 0 );

$popup = file_get_contents( WP_PLUGIN_DIR . '/breo-social-login/includes/class-breo-sl-popup.php' );
$ui    = file_get_contents( WP_PLUGIN_DIR . '/breo-social-login/includes/class-breo-sl-ui.php' );
$oauth = file_get_contents( WP_PLUGIN_DIR . '/breo-social-login/includes/class-breo-sl-oauth.php' );
...
t( 'the token-accepting endpoint is gone', false === strpos( $popup, 'breo_sl_fb_token' ), true );
t( '  ...and is no longer registered', has_action( 'wp_ajax_nopriv_breo_sl_fb_token' ), false );
...
t( 'the start URL itself carries no popup flag',
	false === strpos( BREO_SL_OAuth::start_url( 'google' ), 'popup' ), true );
...
$onetap = file_get_contents( WP_PLUGIN_DIR . '/breo-social-login/includes/class-breo-sl-onetap.php' );
...
$css = file_get_contents( WP_PLUGIN_DIR . '/breo-social-login/assets/breo-social-login.css' );
t( 'the button row centres its items', false !== strpos( $css, 'align-items:center;' ), true );
t( 'the Google host is not inline', false !== strpos( $css, '.breo-sl-gbtn{' ) && false !== strpos( $css, 'display:inline-flex' ), true );
...
t( 'our buttons resize to match the Google button', false !== strpos( $popup, "row.style.setProperty('--breo-sl-size'" ), true );
...
t( 'the measured width is published too', false !== strpos( $popup, "'--breo-sl-gw'" ), true );
...
t( 'our labelled button uses the measured width', false !== strpos( $css, 'width:var(--breo-sl-gw,100%)' ), true );
...
t( 'label_style defaults to continue_with', BREO_SL_Options::defaults()['label_style'], 'continue_with' );
foreach ( array( 'continue_with', 'signin_with', 'signup_with', 'signin' ) as $v ) {
	t( "  '$v' is accepted", BREO_SL_Options::sanitize( array( 'label_style' => $v ) )['label_style'], $v );
}
t( 'a bare "google" is rejected', BREO_SL_Options::sanitize( array( 'label_style' => 'google' ) )['label_style'], 'continue_with' );
t( 'junk falls back to the default', BREO_SL_Options::sanitize( array( 'label_style' => '<x>' ) )['label_style'], 'continue_with' );
...
t( 'google_button defaults to native', BREO_SL_Options::defaults()['google_button'], 'native' );
t( 'custom is accepted', BREO_SL_Options::sanitize( array( 'google_button' => 'custom' ) )['google_button'], 'custom' );
t( 'junk falls back to native', BREO_SL_Options::sanitize( array( 'google_button' => 'gapi' ) )['google_button'], 'native' );
...
t( 'brand-only wording is accepted', BREO_SL_Options::sanitize( array( 'label_style' => 'brand' ) )['label_style'], 'brand' );
...
t( 'wide buttons get their own radius variable', false !== strpos( $ui, '--breo-sl-radius-w:' ), true );
t( '  ...and the stylesheet uses it', false !== strpos( $css, 'border-radius:var(--breo-sl-radius-w,999px)' ), true );
...
t( '  ...both heights come from the measurement',
	false !== strpos( $css, 'height:var(--breo-sl-size,46px);min-height:var(--breo-sl-size,46px)' ), true );

t( 'the row is flagged once Google draws a button', false !== strpos( $popup, "classList.add('breo-sl-native')" ), true );
t( '  ...and our hover then matches theirs', false !== strpos( $css, '.breo-sl-native .breo-sl-btn:hover' ), true );
...
t( 'Facebook still gets the window', false !== strpos( $popup, 'breo_sl_login' ), true );
...
t( 'the library is no longer gated on onetap_enabled',
	false === strpos( $onetap, "if ( ! BREO_SL_Options::get( 'onetap_enabled' ) || ! BREO_SL_Options::provider_ready( 'google' ) ) {" ), true );
...
set_opts( BREO_SL_Options::defaults() );
