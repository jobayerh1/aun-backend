...
class Breo_Alpha_OTP_Session {

	const COOKIE       = 'breo_alpha_otp_sess';
	const TRANSIENT    = 'breo_alpha_otp_';      // + token
	const RL_PHONE     = 'breo_alpha_otp_rlp_';  // + md5(phone)
	const RL_IP        = 'breo_alpha_otp_rli_';  // + md5(ip)
	 ...
	 * @param array  $extra       Extra payload keys (e.g. 'new' => true for a number without an account).
	 */
	public function store_otp( $canonical, $otp, $user_ids, $expiry, $resend_wait, $extra = array() ) {
		$now      = time();
		$existing = $this->get_data();
		$resends  = isset( $existing['resends'], $existing['phone'] ) && $existing['phone'] === $canonical ? (int) $existing['resends'] : 0;
