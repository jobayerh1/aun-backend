...
| **Verify the AAB is signed with the upload key ✅ automatic** | `key.properties` absent ⇒ the release build REFUSES to run (the one-way-door guard in `build.gradle.kts`); `build-aun-app-aab.cmd` stops a second earlier with a friendlier message. Neither is the last line of defence: `build-aun-app-aab.cmd` now reads the certificate back **out of the finished file** — the only check a stale `key.properties` or a Gradle cache cannot fool — prints Owner + SHA-256, and hard-stops if it says `Android Debug`. ⚠️ Note the same command does **not** work on the APK: `keytool -printcert -jarfile AUN-Care-Bangladesh.apk` answers *"Not a signed jar file"*, because modern APKs carry only signature scheme v2/v3 and keytool reads v1 JAR signatures. Bundles are JAR-signed, so it works there. For an APK use `apksigner verify --print-certs`. |
